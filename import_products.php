...
foreach ($products as $product) {
  $stmt = $conn->prepare("INSERT INTO products (id, name, image, description, price) VALUES (?, ?, ?, ?, ?)");
  $stmt->bind_param("isssd", $product['id'], $product['name'], $product['image'], $product['description'], $product['price']);
  $stmt->execute();
}
...
echo "Товары успешно добавлены в базу данных.";
?> -->
